...
def get_review_requests(args):
  ...
  if "user" not in user_config:
    print("A user has not been configured. Please use 'pyprs set --user <GITHUB_USER_ID>' to configure one.")
    return
  
  if "token" not in user_config:
    print("An access token has not been configured. Please use 'pyprs set --token <GITHUB_ACCESS_TOKEN>' to configure one.")
    return
