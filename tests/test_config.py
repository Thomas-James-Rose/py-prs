...
def test_set_user(monkeypatch):
  ...
  output = run_and_get_console_output(lambda : py_prs.config.set(argparse.Namespace(user=[user], token=None)))
  
  assert output == Template("{'user': ['$user_id']}\n").substitute(user_id=user)
...
def test_set_access_token(monkeypatch):
  ...
  output = run_and_get_console_output(lambda : py_prs.config.set(argparse.Namespace(user=None, token=[token])))
  
  assert output == Template("{'token': ['$access_token']}\n").substitute(access_token=token)